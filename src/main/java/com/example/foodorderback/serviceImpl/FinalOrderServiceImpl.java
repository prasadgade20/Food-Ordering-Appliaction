	...
	public Long makeFinalOrder(OrderItemDTO orderItemDTO) {
		...
		try {
			...
			for(ItemFromCartDTO itc: orderItemDTO.getItemsFromCart()) {
				...
				// I will not store the image in the database when creating an order item to avoid filling the database with images, as they take up a lot of space during deployment.
				// Instead, the image will be loaded from the meal table when sending the order item to the client.
